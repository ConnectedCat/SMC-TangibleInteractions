...
void setup(){
  ...
  // THIS TIME WE'RE GOING TO TRACK THE BRIGHTNESS ON THE SCALE FROM 0 TO 255
  // TO ENSURE COMPATIBILITY WITH ARDUINO LEDS
  ...
  for(int i = 0; i < numberOfButtons; i++){
   // add a new vector into every slot of the positions array
    ...
    // THIS TIME THE BRIGHTNESS IS CALCULATED ON A 0-255 SCALE
...
void mouseClicked(){
  ...
  for(int i = 0; i < numberOfButtons; i++){
    ...
    if(dist(buttonPositions[i].x, buttonPositions[i].y, mouseX, mouseY) < buttonRadius){
      // read the brightness of the color of this button
      float thisBrightness = brightness(buttonColors[i]);
      // convert the resulting value to an integer
      int valToSend = int(thisBrightness);
      // send it down the serial port
